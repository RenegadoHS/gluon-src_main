...
ConVar  alyx_darkness_force( "alyx_darkness_force", "0", FCVAR_CHEAT | FCVAR_REPLICATED );
	...
	void CHalfLife2::InitDefaultAIRelationships( void )
	{
		...
		CBaseCombatCharacter::SetDefaultRelationship(CLASS_WASTE_SCANNER,		CLASS_COMBINE_HUNTER,	D_HT, 0);
...
{
	if( pInfo->GetDamageType() & (DMG_DROWN|DMG_CRUSH|DMG_FALL|DMG_POISON|DMG_CLUB) )
	{
...
CAmmoDef *GetAmmoDef()
{
	...
	if ( !bInitted )
	{
		...
		def.AddAmmoType("MP5_Grenade",		DMG_BURN,					TRACER_NONE,			"sk_plr_dmg_mp5_grenade",	"sk_plr_dmg_mp5_grenade",	"sk_max_smg1_grenade",	0,							0 );
		...
		def.AddAmmoType("ammo_ar3",			DMG_SHOCK,				TRACER_LINE,			0, 0, 30, 0, 0 );
#endif // HL2_EPISODIC
