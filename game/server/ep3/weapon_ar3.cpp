...
//
// Purpose:		AR3 - the city was lousy with them
//
// $NoKeywords: $
//=============================================================================//
#include "cbase.h"
...
#ifndef CLIENT_DLL
ConVar weapon_ar3_rate_of_fire( "weapon_ar3_rate_of_fire", "10" );
ConVar weapon_ar3_mass_limit( "weapon_ar3_mass_limit", "600" );
ConVar weapon_ar3_alt_fire_range( "weapon_ar3_alt_fire_range", "250" );
ConVar weapon_ar3_ping_delay( "weapon_ar3_ping_delay", "1" );
ConVar weapon_ar3_ping_radius( "weapon_ar3_ping_radius", "1200" );
ConVar weapon_ar3_penetrate_depth( "weapon_ar3_penetrate_depth", "24" );
ConVar weapon_ar3_exit_blast_radius( "weapon_ar3_exit_blast_radius", "100" );
ConVar weapon_ar3_echo_sound_duration( "weapon_ar3_echo_sound_duration", "0.25" );
#endif
...
// CWeaponAR3
//-----------------------------------------------------------------------------
class CWeaponAR3 : public CBaseHLCombatWeapon
{
	DECLARE_CLASS( CWeaponAR3, CBaseHLCombatWeapon );
public:
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CWeaponAR3::FireHitscanBolt()
{
	if( !HasAnyAmmo() )
	{
		WeaponSound( EMPTY );
	...
	if( pObjectStruck->IsWorld() )
	{
		...
		UTIL_TraceLine( vecStart, vecStart + vecAiming * weapon_ar3_penetrate_depth.GetFloat(), MASK_SHOT, pIgnoreEnt, COLLISION_GROUP_NONE, &tr );
		...
		tr.endpos = vecStart + vecAiming * (weapon_ar3_penetrate_depth.GetFloat() * tr.fractionleftsolid);
	}
	else
	{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CWeaponAR3::Equip( CBaseCombatCharacter *pOwner )
{
	BaseClass::Equip( pOwner );

	pOwner->GiveAmmo( 3, "ammo_ar3" );
}
...
void CWeaponAR3::Precache( void )
{
	...
	PrecacheModel(PENETRATING_BOLT_MODEL);
...
void CWeaponAR3::PrimaryAttack( void )
{
	...
	//FirePenetratingBolt(); //BUGBUG: Conflicts with PrimaryAttack
	FireHitscanBolt();

	float flRateOfFireDelay = 1.0f / weapon_ar3_rate_of_fire.GetFloat();
...
//-----------------------------------------------------------------------------
bool CWeaponAR3::CanPenetratingBoltReachTarget( CBaseEntity *pTarget )
{
	trace_t tr;
	CBasePlayer *pPlayer = ToBasePlayer( GetOwner() );
	...
	if( flDistFromWall > weapon_ar3_exit_blast_radius.GetFloat() )
		return false;
	...
	if( flSolidThickness > weapon_ar3_penetrate_depth.GetFloat() )
	{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CWeaponAR3::CanConsumeObject( CBaseEntity *pObject )
{
	...
	if( pPhysObject->GetMass() > weapon_ar3_mass_limit.GetFloat() )
	{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CWeaponAR3::ConsumeObject( CBaseEntity *pObject )
{
	...
	GetOwner()->GiveAmmo( 1, "ammo_ar3" );
}

//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CWeaponAR3::SendPing()
{
	EmitSound( SONAR_PING_SOUND );

	DoPingEffect();
	DoPingSearch();
	m_flTimeNextPing = gpGlobals->curtime + weapon_ar3_ping_delay.GetFloat();
}

//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CWeaponAR3::DoPingEffect() 
{
	CBroadcastRecipientFilter filter;

	te->BeamRingPoint( filter, 0, GetAbsOrigin(),	//origin
		8.0f,	//start radius
		weapon_ar3_ping_radius.GetFloat() * 2.0f,		//end radius
...
void CWeaponAR3::DoPingSearch() 
{
	...
	float flMaxDistSqr = Square( weapon_ar3_ping_radius.GetFloat() );
	int iFoundTargets = 0;
	...
	m_flEchoSoundDelay = weapon_ar3_echo_sound_duration.GetFloat() / ((float)iFoundTargets);
}

//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CWeaponAR3::PingEntity( CBaseEntity *pEntity )
{
	CBasePlayer *pPlayer = ToBasePlayer( GetOwner() );
...
//-----------------------------------------------------------------------------
CBaseEntity *CWeaponAR3::DetectObject()
{
	trace_t tr;
	...
	UTIL_TraceLine( vecSrc, vecSrc + vecAiming * weapon_ar3_alt_fire_range.GetFloat(), (MASK_SHOT|CONTENTS_GRATE), pPlayer, COLLISION_GROUP_NONE, &tr );
