...
ConVar pbr_notusecubemap("pbr_notusecubemap", "0", FCVAR_ARCHIVE, "Should PBR not use cubemaps? (Only a good option if the map has next to no cubemaps)");
...
BEGIN_SHADER_PARAMS
	...
	SHADER_PARAM(ENVMAP, SHADER_PARAM_TYPE_ENVMAP, "env_cubemap", "Set the cubemap for this material.")
	SHADER_PARAM(MRAOTEXTURE, SHADER_PARAM_TYPE_TEXTURE, "pbr/base_nrao", "Texture with metalness in R, roughness in G, ambient occlusion in B.")
