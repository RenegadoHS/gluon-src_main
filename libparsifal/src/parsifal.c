...
{	
	if (chSize == 1) return ISMAPCH(nameStartAscii, *ch);
	if (chSize == 2) return utf8_isNmstrt2(ch);
	if (chSize == 3) return utf8_isNmstrt3(ch);
	return 0;
}

int XMLAPI XMLIsNameChar(XMLCH *ch, int chSize)
{	
	if (chSize == 1) return ISMAPCH(nameAscii, *ch);
	if (chSize == 2) return utf8_isName2(ch);
	if (chSize == 3) return utf8_isName3(ch);
	return 0;
}
...
{	
	if (!parser || !PREADER->buf || !PREADER->bytesavail) return -1;
	else {
		int pos = PREADER->pos - PREADERDATA->col;
		if (pos < 0) return -1;
